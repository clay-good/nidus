...
- **Review status:** 28 parameters `verified` (a human checked the source PDF); 38 `pending_human_review` (the stored value was located in a real source by automated review, with a verbatim quote on the record, but no human has signed off — see [`data/validation/`](data/validation/)); 1 `contested`; the remaining 176 `unverified` (illustrative central value from the literature, source not yet confirmed). Automated review never sets `verified` — that stays a human action.
